...
// Frontend-safe guide sections (internal already stripped) — imported by the guide.
const GUIDE_OUT_FILE = join(ROOT, 'src', 'generated', 'guide.ts');
...
// Frontend-safe guide module. Guide sections are guest-facing (internal already
// stripped during collection), so it is safe to ship in the client bundle.
function emitGuide(): string {
  const j = (v: unknown) => JSON.stringify(v, null, 2);
  const guideEntries = properties.map((p) => `  '${p.slug}': ${j(p.guide)},`).join('\n');

  return `// AUTO-GENERATED by scripts/generate-content.ts — DO NOT EDIT.
// Frontend-safe guide sections (no chatbot facts, internal stripped). Source: content/properties/**
/* eslint-disable */
import type { GuideSection, Locale } from '../content/schemas';

export const guideSections: Record<string, Partial<Record<Locale, GuideSection[]>>> = {
${guideEntries}
};

const GUIDE_FALLBACKS: Locale[] = ['de', 'en'];

/**
 * Resolve a property's guide sections for a locale (fallback locale → de → en),
 * sorted by sortOrder. Returns [] when none exist.
 */
export function getGuideSections(slug: string, locale: string): GuideSection[] {
  const byLocale = guideSections[slug];
  if (!byLocale) return [];
  for (const l of [locale as Locale, ...GUIDE_FALLBACKS]) {
    const s = byLocale[l];
    if (s && s.length) return [...s].sort((a, b) => a.sortOrder - b.sortOrder);
  }
  return [];
}
`;
...
writeFileSync(GUIDE_OUT_FILE, emitGuide(), 'utf8');
console.log(`✔ Generated ${OUT_FILE} + ${REC_OUT_FILE} + ${HERO_OUT_FILE} + ${GUIDE_OUT_FILE} from ${properties.length} propert${properties.length === 1 ? 'y' : 'ies'} (${slugs.join(', ')})`);
